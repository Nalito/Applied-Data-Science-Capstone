...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                dcc.Dropdown(id='site-dropdown', options=[{'label': 'All Sites', 'value': 'ALL'}, {'label': 'site1', 'value': 'site1'}], value='ALL', placeholder='Select a Launch site here', searchable=True),
                                html.Br(),
...
@app.callback(Output(component_id='success-payload-scatter-chart', component_property='figure'), [Input(component_id='site-dropdown', component_property='value'), Input(component_id="payload-slider", component_property="value")])
    def get_scatter_chart(entered_site):
        if entered_site == 'ALL':
            fig = px.scatter(data, values=['Payload Mass (kg)', 'class'], color='Booster Version Category')
            return fig
        else:
            data1 = data[[entered_site]]
            fig = px.scatter(data, values=['Payload Mass (kg)', 'class'], color='Booster Version Category')
            return fig
# Run the app
